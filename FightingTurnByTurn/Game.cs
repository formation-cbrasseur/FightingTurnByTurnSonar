...
namespace FightingTurnByTurn
{
    public class Game
    {
        ...
        public Character Turn()
        {
            ...
            if (otherPlayer.Characters.TrueForAll(charac => !charac.Alive) || player.Characters.TrueForAll(charac => !charac.Alive))
            {
                HasEnded = true;
                SetWinner();
                LogEndGame();
        ...
        {
            if (HasEnded)
                Winner = Players.First(charac => charac.Characters.Exists(characters => characters.Alive));
        }
